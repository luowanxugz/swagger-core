...
public class ReaderUtils {
    ...
    private static boolean isContext(List<Annotation> annotations) {
        for (Annotation annotation : annotations) {
            if (JaxRsAnnotationUtils.isJaxRsAnnotation(annotation, "Context", "core")) {
                return true;
            }
        }
        return false;
    ...
    public static String getPath(Annotation classLevelPath, Annotation methodLevelPath, String parentPath, boolean isSubresource) {
        ...
        if (classLevelPath != null && !isSubresource) {
            appendPathComponent(JaxRsAnnotationUtils.getAnnotationValue(classLevelPath), b);
        }
        if (methodLevelPath != null) {
            appendPathComponent(JaxRsAnnotationUtils.getAnnotationValue(methodLevelPath), b);
        }
    ...
    public static String extractOperationMethod(Method method, Iterator<OpenAPIExtension> chain) {
        String httpMethod = JaxRsAnnotationUtils.extractHttpMethod(method);
        if (httpMethod != null) {
            return httpMethod;
        }
        Annotation httpMethodAnnotation = JaxRsAnnotationUtils.getAnnotation(method, "HttpMethod");
        if (httpMethodAnnotation != null) {
            return JaxRsAnnotationUtils.getAnnotationValue(httpMethodAnnotation).toLowerCase();
        }
    ...
    public static String getHttpMethodFromCustomAnnotations(Method method) {
        for (Annotation methodAnnotation : method.getAnnotations()) {
            Annotation httpMethodMeta = JaxRsAnnotationUtils.getAnnotation(methodAnnotation.annotationType(), "HttpMethod");
            if (httpMethodMeta != null) {
                return JaxRsAnnotationUtils.getAnnotationValue(httpMethodMeta).toLowerCase();
            }
        }
        return null;
    }
